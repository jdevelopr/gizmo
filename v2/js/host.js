...
export function startHost(show) {
  ...
  function showResults() {
    ...
    const top = res.length ? engine.players.get(res[0].seat) : null;
    $('#results-sub').textContent = top
      ? `${res[0].name} shipped the most, on a ${top.f.claim}x${top.f.claim} plot, `
        + `filling ${top.filled} of ${engine.cfg.rounds} orders.`
      : '';
    show('results');
    host.over(res);
  }
  ...
  function noteFor(p, rank) {
    ...
    // Mid-round the useful number is progress against your own order, not rank.
    const tgt = p.order?.target || 0;
    if (engine.phase === 'run' && tgt) {
      const pct = Math.min(999, Math.round((p.f.income / tgt) * 100));
      return `ORDER ${pct}%` + (rank === 1 ? ' · LEADING' : '');
    }
