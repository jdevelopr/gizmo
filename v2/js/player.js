...
export function createController({ send }) {
  ...
  function paintSel() {
    ...
    // Throughput, spelled out. Ratios are the whole game once a floor has more
  ...
  function paintHud() {
    ...
    // The order board. Nothing on this floor is going to be taken away from you,
    // so the question the HUD asks is the only one that matters now: is what you
    // have built shipping more than it ever has?
    const tgt = hud.order?.target || 0;
    const got = view.n || 0;
    const pct = tgt ? Math.min(100, Math.round((got / tgt) * 100)) : 0;
    const ord = $('#pad-order');
    if (ord) {
      ord.hidden = hud.ph === 'lobby' || hud.ph === 'over';
      ord.dataset.met = got >= tgt && tgt > 0 ? 'on' : 'off';
      $('#order-fill').style.width = pct + '%';
      $('#order-text').textContent = tgt
